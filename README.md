...
Multiple interactive windows will open:
  ...
  * **`Imagine` (Original Image)**
  * **`Preview pista` (Result Preview)**
  * **`Culori selectate` (Control Panel)**
  * **`Flood Preview`**
...
| :--- | :--- | :--- | :--- |
| **`Culori selectate`** | **Remove Color** | **Left Click** on a color swatch | Removes a previously included color. |
| **`Preview pista`** | **Add Flood Seed** | **Left Click** anywhere | Adds a point as a seed for the **Flood Fill** operation, which runs on the **color-filtered** image. |
...
| :--- | :--- | :--- |
| `Tol Inc` | `Preview pista` | Adjusts the **Tolerance** for color inclusion (`tol_include`). |
| `Open Ker` | `Preview pista` | Adjusts the kernel size for the initial **Opening** operation (`ker_open`). |
| `Close Ker` | `Preview pista` | Adjusts the kernel size for the initial **Closing** operation (`ker_close`). |
| `Flood Tol` | `Flood Preview` | Adjusts the **Tolerance** for the flood-fill operation (`flood_tolerance`). |
| `Flood Open` | `Flood Preview` | Adjusts the kernel size for **Opening** *after* flood fill (`flood_ker_open`). |
| `Flood Close` | `Flood Preview` | Adjusts the kernel size for **Closing** *after* flood fill (`flood_ker_close`). |
