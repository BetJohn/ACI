...
def get_perpendicular_segments(image_path):
    step = 13  # Increased step slightly for visibility in debug
    ...
    for i in range(0, num_points, step):
        ...
        # --- 1. CIRCULAR INDEXING (Essential for Flow consistency) ---
        # Gets neighbors strictly in order: [previous ... current ... next]
        ...
        # --- 3. DIRECTION CORRECTION (New Logic) ---
        # Calculate the "flow" vector: Direction from start of window to end of window
        # neighbors[-1] is the point "ahead", neighbors[0] is "behind"
        flow_vector = neighbors[-1] - neighbors[0]
        
        # Normalize flow vector just for the dot product safety (optional but good practice)
        dist = np.linalg.norm(flow_vector)
        if dist > 0:
            flow_vector = flow_vector / dist
            
            # Dot product: If < 0, vectors are opposing
            if np.dot(tangent, flow_vector) < 0:
                tangent = -tangent  # Flip the tangent to match flow

        # --- 4. CALCULATE NORMAL ---
        # Now that tangent is consistently "forward", normal will be consistently "left" (or right)
        ...
        # Draw tangent (Green) - Should now always point "forward" along the track
...
if img_result is not None:
    ...
    plt.title(f"Track Analysis - Consistent Tangents")
    plt.axis('off')
    plt.show()